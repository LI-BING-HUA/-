...
> 超簡記法：**開始取「晚」（被前面拖，max）、截止取「早」（被後面逼，min）。**
 
| | 怎麼取 | 被誰影響 |
|--|--------|---------|
| 有效開始 release | 取**晚**的 max | 前面的人（前驅）|
| 有效截止 deadline | 取**早**的 min | 後面的人（後繼）|
 
**課本例**：B 開始時間自己寫 1，但前面的 A 是 2 → B 有效開始 = max(1,2) = **2**；B 截止自己寫 12，但後面的人最早要 8 → B 有效截止 = min(12,…,8) = **8**。
 
**算完有什麼好處？（單處理器 + 可搶佔）**：算好有效時間後就**不用再管順序了**，把每個工作當成「各做各的、互不相干」直接用 EDF 排就好。因為順序已偷偷藏進時間裡（前面的人截止較早會先排、後面的人開始較晚不會太早做）。萬一排出來順序還是錯 → 把那兩個 job **swap（對調位置）** 就變合法。
