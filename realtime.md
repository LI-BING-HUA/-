...
<img width="1360" height="920" alt="flight_control_taskgraph_a_no_sync" src="請貼上你截圖的網址" />

**(b) 同步 → precedence 實線箭頭**（consumer 必須等 producer 這次做完）

<img width="1360" height="920" alt="flight_control_taskgraph_b_sync" src="請貼上你截圖的網址" />
...
- 常見優先權指派：FIFO/LIFO（依 release time）、SETF/LETF（依執行時間）。
- **Preemptive（可搶佔）vs Non-preemptive（不可搶佔）**：
  - 一般情況**沒有定論**誰比較好。
  - 特例（所有 job 同 release time、忽略搶佔成本）：preemptive 的 **makespan**（最後完成 job 的時間）≤ non-preemptive。
  - 兩處理器：non-preemptive 的最小 makespan ≤ 4/3 × preemptive 的最小 makespan。
...
當 job 的 release/deadline 跟 precedence 矛盾時（如 release 比後繼還晚），要先導出**一致的有效時間**。單處理器規則：

- **有效 release time**：
  - 無前驅 → = 自己的 release time
  - 有前驅 → = **max(自己的 release, 所有前驅的有效 release)**（往前推，不能比前驅早）
- **有效 deadline**：
  - 無後繼 → = 自己的 deadline
  - 有後繼 → = **min(自己的 deadline, 所有後繼的有效 deadline)**（往前收，不能比後繼晚）

> 記法：**release 取 max（被前驅卡住、只能更晚）、deadline 取 min（被後繼卡住、只能更早）**。
> 課本例：J3 給定 release=1，但前驅 J1 有效 release=2 → J3 有效 release = max(1,2)=**2**；J3 給定 deadline=12，後繼有效 deadline 最小為 8 → J3 有效 deadline = min(12,…,8)=**8**。

**單處理器 + 可搶佔的關鍵性質**：用有效時間後，可**暫時忽略 precedence**、把所有 job 當獨立的來排（萬一排出來違反順序，**swap 交換**兩個 job 即可變成合法排程）。
...
EDF、LST 只在「單處理器 + 可搶佔」最佳。下列三種情況**失效**：

1. **不可搶佔（non-preemptive）**：
   - 例：J1(r0,e3,d10)、J2(r2,e6,d14)、J3(r4,e4,d12)。EDF 下 J1 完成時只有 J2 ready 先跑 J2，J3 來了卻不能搶 → **J3 miss**。
   - 但「t=3 故意讓處理器**閒置**等 J3」可行 → 然而 priority-driven **永不故意閒置** → 所以**沒有任何 non-preemptive priority-driven 演算法是最佳的**。

2. **多處理器（>1 processor）**：
   - 例：J1(e1,d1)、J2(e1,d2)、J3(e5,d5)，release 全 0，2 個處理器。EDF 先排 J1、J2（deadline 早）→ **J3 miss**。
   - 改用 **LST**（slack：J1=0, J2=1, J3=0）→ 給 J3 高優先 → 可行。但 **LST 在多處理器也不是最佳**（Problem 4.4 反例）。

> 一句話：**EDF/LST 只在「單 CPU + 可搶佔」最佳；一旦「不可搶佔」或「多處理器」就可能 miss，要靠反例記住。**

### Clock-driven vs Priority-driven（優缺點）
- **Priority-driven 優點**：好實作、run-time 負擔小、不需事先知道所有參數 → 適合需求會變的應用。
- **Priority-driven 缺點（為何硬即時少用）**：job 參數一變，時序行為**不確定**，**難驗證**所有 deadline 都會達成。
- **Graham 異常（anomaly）**：priority-driven 多處理器系統會出現反直覺現象——**增加處理器、減少執行時間、放寬 precedence，反而可能使某些 job 變更晚完成 / miss deadline**（4.5 那幾題就是在算這個）。
