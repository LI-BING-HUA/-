...
**題目（Figure 1-3 飛控系統）**：每 1/180 秒一個 cycle。

① 驗證感測資料/選資料源（每 cycle）；

② 30-Hz avionics（鍵盤、座標轉換、追蹤更新，每6 cyc）；

③ 30-Hz 外迴路控制律（pitch/roll/yaw outer，每6 cyc）；

④ 90-Hz 內迴路控制律（inner pitch、inner roll+coll，每2 cyc，**用 30-Hz 計算+avionics 的輸出**）；

⑤ inner yaw 控制律（**用 90-Hz 的輸出**）；

⑥ output commands；⑦ built-in-test；⑧ 等下一個 cycle。
...
|--|---------|-----------|--------|------|
| (b) | 原始 | t=3(剛好趕上) | 12 | ✅ |
| (c) | 改可搶佔 | t=3 起跑但被搶佔 | 16 | ❌ |
| (d) | 加到 4 台 | t=6 | 15 | ❌ |
| (e) | 每個 e −1 | t=5 | 13 | ❌ |

### 🆕 練習題 4.6（≥2 處理器上沒有最佳的線上排程器）⭐
**先分清楚兩個角色**
 
| | 是什麼 | 能不能預知未來 |
|--|--|--|
| **線上(on-line)** | 真實排程器,邊走邊決定、做了回不去 | ❌ 只看得到「已來」的工作 |
| **Clairvoyant(預知)** | **不存在的理論對照組**,當「最好能多好」的尺 | ✅ 一開始就知道未來所有工作 |

**(a) 打「有做 J₃」**:對手放 J₄、J₅(e1, d2, **t=1 來**)
- [0,1) 一台拿去做 J₃ → J₁J₂ 還剩 ≥1
- [1,2] 要塞 (J₁J₂剩≥1)+J₄1+J₅1 = **≥3 > 容量2** → miss ❌
- → clairvoyant 該選「**不做 J₃**」:[0,1]做完J₁J₂、[1,2]給J₄J₅、J₃留[2,4] ✅

**(b) 打「沒做 J₃」**:對手另放 J₄、J₅(**e2, d4, t=2 來**)
- [0,1) 沒碰 J₃ → J₃ 到 t=2 還剩 ≥1
- [2,4] 要塞 (J₃剩≥1)+J₄2+J₅2 = **≥5 > 容量4** → miss ❌

| (b) 線上(沒先做J₃) | 0–1 | 1–2 | 2–3 | 3–4 |
|--|--|--|--|--|
| P1 | J₁ | J₃ | J₄ | J₄ |
| P2 | J₂ | 閒 | J₅ | J₅ |
 
→ J₃ 差 1 單位、[2,4] 被占滿 → **J₃ miss ❌**
 
| (b) Clairvoyant(先做J₃) | 0–1 | 1–2 | 2–3 | 3–4 |
|--|--|--|--|--|
| P1 | J₁ | J₂ | J₄ | J₄ |
| P2 | J₃ | J₃ | J₅ | J₅ |
 
→ J₃ 在 t=2 做完、J₄J₅ 占滿[2,4] → **全達成 ✅**

### 🆕 練習題 4.7（有效時間 + 區間需求檢查 + release jitter）⭐

**題目**:看 Figure 4-3 那組工作,假設**所有工作執行時間相同**。
- **(a)** 在單處理器上仍能可行排程的前提下,這個共同執行時間**最大能是多少**?說明理由。
- **(b)** 假設 J₁、J₂ 的 release time 會抖動(jitter):J₁ 最早 0、最晚 3,J₂ 最晚 1。

<img width="486" height="155" alt="image" src="https://github.com/user-attachments/assets/3aa04f2e-eeaa-443d-8508-84995d3298ec" />

**(a)**
**Step 1 — 算有效時間**(r\* 從前往後取 max、d\* 從後往前取 min)
 
| Job | 給定(r,d) | r\* | d\* |
|--|--|--|--|
| J₁ | (2,10) | 2 | min(10, **J₃=8**)=8 |
| J₂ | (0,7) | 0 | min(7, 8)=7 |
| J₃ | (1,12) | max(1,2,0)=2 | min(12, **J₄=9, J₅=8**)=8 |
| J₄ | (4,9) | max(4,2)=4 | 9 |
| J₅ | (1,8) | max(1,2)=2 | 8 |
| J₆ | (0,20) | max(0,4,2)=4 | 20 |
| J₇ | (6,21) | max(6,4,2)=6 | 21 |

**Step 2 — 我的解法(刪逃生口 → 框最緊窗口)**

- 1. **先算21秒7個job，每個job平均3秒**
- 2. **可刪 J₆ J₇ ， 因為9~21秒讓他們隨便做**
- 3. **剩 J₁ ~ J₅ ， 9秒5個job，平均一個job 1.8秒，可刪J₂**
- 4. **剩2~9秒四個job，平均一個job 1.75秒**
- 5. **沒得刪，結束**

**(b) release jitter — 方法完全同 (a),只多一步:抖動取「最晚 release」當最壞情況**
| Job | 新 r\* | d\* |
|--|--|--|
| J₁ | **3** | 8 |
| J₂ | **1** | 7 |
...
> **(5.2) 易錯重點**:課本正式寫法是 `⌊pᵢ/f⌋ − pᵢ/f = 0`(⌊ ⌋ = 無條件捨去),意思就是「pᵢ/f 是整數」= **f 整除 pᵢ**。
> - 看的是 **pᵢ ÷ f 的「商」是不是整數(餘數 0)**,**商是整數就合法**;f 自己是不是小數**無所謂**。
> - 例:pᵢ=5、f=**2.5** → 5/2.5 = 2.0(整數)→ **合法**;pᵢ=5、f=2 → 2.5(有小數)→ **不合法**。(週期/執行時間本來就常出現小數,如 T2(5,1.8)。)
...
- 已經被 accept 的 sporadic job,用 EDF 排(deadline 早的先做),在週期工作做完後的 slack 裡跑。

#### 5.6.4 Cyclic EDF 的最佳性
- EDF 在「單處理器 + 可搶佔」是最佳的(Theorem 4.1) —— cyclic EDF 的最佳性就是從這條推來的。

#### 📌 5.6 觀念統整(會考的就這張)
**三種工作類型**
 
| 類型 | release | deadline | 怎麼排 |
|--|--|--|--|
| Periodic | 固定週期、離線已知 | 硬 | 事先排進 cyclic schedule |
| Aperiodic | 隨機 | 軟/無 | slack stealing 撿空檔 |
| **Sporadic** | **隨機** | **硬** | **acceptance test 動態收/拒** |

**Acceptance test 兩個必背「為什麼」**
 
| 問 | 答 |
|--|--|
| 為什麼要測? | 硬 deadline 不能事後才發現 miss → 必須**事前保證**塞得下 |
| 為什麼立刻拒不先收? | 早拒 → 應用程式有時間做 **recovery**,不是拖到 miss 才知道 |
 
> 測試邏輯(懂即可,不背符號):**需要時間 e ≤ deadline 前總空檔 σc → 收,否則拒**(= 需求 ≤ 可用空間,同 4.7 窗口檢查)。
 
**跨章同一條線(觀念題最愛)**:看不到未來 → 收前先測能否塞下 → 塞不下就早拒。
 
| 出處 | 名字 |
|--|--|
| Ch2 | admission request(hard task 先 pass) |
| 4.6 | online vs clairvoyant(沒有最佳線上排程器) |
| 5.6 | acceptance test(e ≤ σc 才收) |
